...
static esp_err_t gm65_scan_handler(httpd_req_t *req) 
{
    gm65.enable_setting_code();
    gm65.set_working_mode(1);
    gm65.set_serial_output();
    gm65.scan_once();
    ...
    while (millis() - start_time < timeout_ms) {
        scan_result = gm65.get_info();
        if (!scan_result.isEmpty() && scan_result.length() > 9) {
            httpd_resp_sendstr(req, scan_result.c_str());
            return ESP_OK;
        }
    ...
    httpd_resp_sendstr(req, "No valid barcode found");
    return ESP_OK;
}
...
static esp_err_t framesize_handler(httpd_req_t *req) 
{
    ...
    if (httpd_req_get_url_query_str(req, buf, buf_len) == ESP_OK) {
        if (httpd_query_key_value(buf, "framesize", framesize_str, sizeof(framesize_str)) == ESP_OK) {
            framesize_t framesize;

            if (!strcmp(framesize_str, "UXGA"))
                framesize = FRAMESIZE_UXGA;
            else if (!strcmp(framesize_str, "QVGA"))
                framesize = FRAMESIZE_QVGA;
            else if (!strcmp(framesize_str, "CIF"))
                framesize = FRAMESIZE_CIF;
            else if (!strcmp(framesize_str, "VGA"))
                framesize = FRAMESIZE_VGA;
            else if (!strcmp(framesize_str, "SVGA"))
                framesize = FRAMESIZE_SVGA;
            else if (!strcmp(framesize_str, "XGA"))
                framesize = FRAMESIZE_XGA;
            else if (!strcmp(framesize_str, "SXGA"))
                framesize = FRAMESIZE_SXGA;
            else if (!strcmp(framesize_str, "HD"))
                framesize = FRAMESIZE_HD;
            else {
                ESP_LOGI(TAG, "Invalid framesize: %s", framesize_str);
...
static esp_err_t cmd_handler(httpd_req_t *req)
{
    ...
    // if (httpd_query_key_value(req->uri, "var", variable, sizeof(variable)) != ESP_OK ||
    //     httpd_query_key_value(req->uri, "val", value, sizeof(value)) != ESP_OK) {
    //     httpd_resp_send_404(req);
    //     return ESP_FAIL;
    // }
...
static esp_err_t connect_handler(httpd_req_t *req) {
    const char *response = "ESP32-CAM connected to Stargel.";
...
static esp_err_t gm65_connect_handler(httpd_req_t *req) {
    uint8_t version = gm65.getSoftwareVersion();

    if (version != 0x00) { 
        char response[50];
        snprintf(response, sizeof(response), "Connected with GM65: V%d.%02d", version / 100, version % 100);
        httpd_resp_send(req, response, HTTPD_RESP_USE_STRLEN);
    } else {
        httpd_resp_send(req, "GM65 not connected or unsupported version", HTTPD_RESP_USE_STRLEN);
    }

    return ESP_OK;
}

static esp_err_t mac_cam_handler(httpd_req_t *req) {
    uint8_t newMACAddress[] = {0x32, 0xAE, 0xA4, 0x07, 0x0D, 0x66};
    char response[18]; 

    esp_err_t wifi_err = esp_wifi_set_mac(WIFI_IF_AP, newMACAddress);
    
    if (wifi_err == ESP_OK) {
        snprintf(response, sizeof(response), "%02X:%02X:%02X:%02X:%02X:%02X",
                 newMACAddress[0], newMACAddress[1], newMACAddress[2],
                 newMACAddress[3], newMACAddress[4], newMACAddress[5]);
        httpd_resp_send(req, response, HTTPD_RESP_USE_STRLEN);
    } else {
        httpd_resp_send(req, "Failed to set MAC address", HTTPD_RESP_USE_STRLEN);
    }
...
void startCameraServer()
{
    ...
    httpd_uri_t vers_uri = {
        .uri = "/gm65_connect",
        .method = HTTP_GET,
        .handler = gm65_connect_handler,
        .user_ctx = NULL
#ifdef CONFIG_HTTPD_WS_SUPPORT
        ,
        .is_websocket = true,
    ...
    httpd_uri_t gm65_scan_uri = {
        .uri = "/gm65_scan",
        .method = HTTP_GET,
        .handler = gm65_scan_handler,
        .user_ctx = NULL
#ifdef CONFIG_HTTPD_WS_SUPPORT
        ,
        .is_websocket = true,
