...
const char set_default[9] = {0x7E, 0x00, 0x08, 0x01, 0x00, 0xD9, 0x55, 0xAB, 0xCD}; //default
const char serial_output[9] = {0x7E, 0x00, 0x08, 0x01, 0x00, 0x0D, 0x00, 0xAB, 0xCD}; //设置串口输出信息
...
const char usb_output[9] = {0x7E, 0x00, 0x08, 0x01, 0x00, 0x0D, 0x01, 0xAB, 0xCD};
...
class GM65_scanner {
  ...
  public:
    ...
    */
    bool enable_setting_code();

    /**
      @brief 关闭扫描设置码功能，通过扫描设置码来进行模块的参数配置。
    */
    bool disable_setting_code();
    ...
    */
    ...
    /**
       ...
       静音模式，1-开启，0-关闭。
    */
    bool set_silent_mode(uint8_t silent_mode);// bit 6

    /**
       @brief 设置GM65的各种模式，工作模式，灯光模式，瞄准模式，指示灯模式。
       LED指示灯模式，扫描成功，蓝色LED灯闪烁。1-开启，0-关闭。
    */
    bool set_LED_mode(uint8_t LED_mode);// bit 7

    /**
       @brief 设置GM65的各种模式，工作模式，灯光模式，瞄准模式，指示灯模式。
       GM65工作模式，0-手动模式，1-命令触发模式，2-连续模式，3-感应模式
    */
    bool set_working_mode(uint8_t working_mode); //bit0-1
    
    /**
       @brief 设置GM65的各种模式，工作模式，灯光模式，瞄准模式，指示灯模式。
       灯光模式，0-无照明，1-普通，2/3-常亮
    */
    bool set_light_mode(uint8_t light_mode); //bit2-3
    ...
    */
    bool set_aim_mode(uint8_t aim_mode); //bit4-5
    ...
    */
    bool set_sleep_mode(uint8_t sleep_mode);
